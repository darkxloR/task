ATTACH TABLE _ UUID '75471246-477c-4793-ba9c-3785071edcda'
(
    ...
    `ProfileEvent_TextIndexDiscardPatternScan` UInt64 COMMENT 'Number of times pattern-based dictionary scan in a text index was discarded because the number of posting lists to read exceeded the threshold.',
    ...
    `ProfileEvent_AzureWriteMicroseconds` UInt64 COMMENT 'Total time spent waiting for Azure write requests.',
    ...
    `ProfileEvent_UserThrottlerBytes` UInt64 COMMENT 'Bytes passed through \'max_network_bandwidth_for_user\' throttler.',
    `ProfileEvent_UserThrottlerSleepMicroseconds` UInt64 COMMENT 'Total time a query was sleeping to conform \'max_network_bandwidth_for_user\' throttling.',
    `ProfileEvent_AllUsersThrottlerBytes` UInt64 COMMENT 'Bytes passed through \'max_network_bandwidth_for_all_users\' throttler.',
    `ProfileEvent_AllUsersThrottlerSleepMicroseconds` UInt64 COMMENT 'Total time a query was sleeping to conform \'max_network_bandwidth_for_all_users\' throttling.',
    ...
    `ProfileEvent_ZooKeeperListRecursive` UInt64 COMMENT 'Number of \'listRecursive\' requests to ZooKeeper.',
    ...
    `ProfileEvent_Shards` UInt64 COMMENT 'The number of shards involved in a query, summed across all distributed tables and table functions. A single host is counted multiple times if it appears in multiple tables. The number counts the total expected number of shards, which includes skipped shards with the `skip_unavailable_shards` setting.',
    ...
    `ProfileEvent_ExternalProcessingCompressedBytesTotal` UInt64 COMMENT 'Number of compressed bytes written by external processing (sorting/aggregating/joining)',
    `ProfileEvent_ExternalProcessingUncompressedBytesTotal` UInt64 COMMENT 'Amount of data (uncompressed, before compression) written by external processing (sorting/aggregating/joining)',
    `ProfileEvent_ExternalProcessingFilesTotal` UInt64 COMMENT 'Number of files used by external processing (sorting/aggregating/joining)',
    ...
    `ProfileEvent_JoinNonJoinedTransformBlockCount` UInt64 COMMENT 'Number of blocks emitted by NonJoinedBlocksTransform.',
    `ProfileEvent_JoinNonJoinedTransformRowCount` UInt64 COMMENT 'Number of non-joined rows emitted by NonJoinedBlocksTransform.',
    `ProfileEvent_JoinDelayedJoinedTransformBlockCount` UInt64 COMMENT 'Number of blocks emitted by DelayedJoinedBlocksWorkerTransform.',
    `ProfileEvent_JoinDelayedJoinedTransformRowCount` UInt64 COMMENT 'Number of rows emitted by DelayedJoinedBlocksWorkerTransform.',
    `ProfileEvent_JoinSpillingHashJoinSwitchedToGraceJoin` UInt64 COMMENT 'Number of times a (Concurrent)HashJoin was switched to GraceHashJoin due to memory limit in SpillingHashJoin.',
    ...
    `ProfileEvent_FilteringMarksWithPrimaryKeyProcessedMarks` UInt64 COMMENT 'Total marks processed during PK analysis.',
    ...
    `ProfileEvent_MergedProjections` UInt64 COMMENT 'Number of projections merged (not rebuilt) during MergeTree merges.',
    `ProfileEvent_RebuiltProjections` UInt64 COMMENT 'Number of projections rebuilt from scratch during MergeTree merges.',
    ...
    `ProfileEvent_MergeCommitMilliseconds` UInt64 COMMENT 'Total time spent for committing merge results (part renaming, checksum verification, ZooKeeper updates)',
    ...
    `ProfileEvent_MutationCommitMilliseconds` UInt64 COMMENT 'Total time spent for committing mutation results (part renaming, checksum verification, ZooKeeper updates)',
    ...
    `ProfileEvent_KeeperReadBatchCount` UInt64 COMMENT 'Number of read request batches processed by Keeper',
    `ProfileEvent_KeeperReadBatchTotalRequests` UInt64 COMMENT 'Total number of read requests processed in batches by Keeper',
    ...
    `ProfileEvent_KeeperReadSnapshot` UInt64 COMMENT 'Number of completed snapshot reads',
    `ProfileEvent_KeeperReadSnapshotObject` UInt64 COMMENT 'Number of snapshot objects sent to followers',
    `ProfileEvent_KeeperReadSnapshotFailed` UInt64 COMMENT 'Number of failed snapshot reads',
    `ProfileEvent_KeeperReadSnapshotDeferred` UInt64 COMMENT 'Number of snapshot reads deferred because the requested snapshot is no longer available locally; NuRaft will retry the latest snapshot.',
    `ProfileEvent_KeeperSnapshotRemoteLoaderErrors` UInt64 COMMENT 'Number of remote read errors in RemoteSnapshotLoader while serving a snapshot to a follower',
    `ProfileEvent_KeeperSaveSnapshotObject` UInt64 COMMENT 'Number of snapshot objects received from leader',
    `ProfileEvent_KeeperSaveSnapshotFailed` UInt64 COMMENT 'Number of failed snapshot saves',
    ...
    `ProfileEvent_KeeperListRecursiveRequest` UInt64 COMMENT 'Number of get children recursive requests',
    ...
    `ProfileEvent_DistrCacheServerUpdates` UInt64 COMMENT 'Distributed Cache event. The number of server switches during read or write because the hash-chosen server changed (e.g. due to server deregistration)',
    ...
    `ProfileEvent_DistrCacheWriteReconnectsAfterTimeout` UInt64 COMMENT 'Distributed Cache write buffer event. The number of reconnects after timeout',
    `ProfileEvent_DistrCacheWriteMicroseconds` UInt64 COMMENT 'Distributed Cache write buffer event. Time spent in WriteBufferFromDistributedCache::writeToFileSegment',
    `ProfileEvent_DistrCacheWriteBytes` UInt64 COMMENT 'Distributed Cache write buffer event. Number of bytes written to distributed cache',
    `ProfileEvent_DistrCacheObjectStorageWriteMicroseconds` UInt64 COMMENT 'Distributed Cache write buffer event. Time spent writing to object storage',
    `ProfileEvent_DistrCacheObjectStorageWriteBytes` UInt64 COMMENT 'Distributed Cache write buffer event. Number of bytes written to object storage',
    ...
    `ProfileEvent_DistrCacheStaleReconnections` UInt64 COMMENT 'Distributed Cache connection event. The number of reconnections due to stale (peer-closed) pooled connections',
    `ProfileEvent_DistrCacheRemoveOutdatedMicroseconds` UInt64 COMMENT 'Distributed Cache connection event. The time spent removing outdated connections from the pool',
    ...
    `ProfileEvent_SharedMergeTreeMergePartsMovedToOudated` UInt64 COMMENT 'How many parts moved to outdated directory',
    ...
    `ProfileEvent_SharedMergeTreeCondemnedPartsLockConflict` UInt64 COMMENT 'How many times we failed to acquire lock because of conflict',
    ...
    `ProfileEvent_SharedMergeTreeReplicaSetUpdateTaskRuns` UInt64 COMMENT 'Number of times updateReplicaSetTask has run',
    ...
    `ProfileEvent_ExecutableUserDefinedFunctionInvocations` UInt64 COMMENT 'Number of executable and executable pool user-defined function invocations.',
    `ProfileEvent_ExecutableUserDefinedFunctionElapsedMicroseconds` UInt64 COMMENT 'Wall clock time spent executing executable and executable pool user-defined functions, in microseconds.',
    `ProfileEvent_ExecutableUserDefinedFunctionUserTimeMicroseconds` UInt64 COMMENT 'User mode CPU time consumed by executable and executable pool user-defined function child processes, in microseconds.',
    `ProfileEvent_ExecutableUserDefinedFunctionSystemTimeMicroseconds` UInt64 COMMENT 'Kernel mode CPU time consumed by executable and executable pool user-defined function child processes, in microseconds.',
    `ProfileEvent_ExecutableUserDefinedFunctionPeakMemoryByteSeconds` UInt64 COMMENT 'Per-process peak memory used by executable and executable pool user-defined function child processes integrated over wall clock time, in byte-seconds.',
    `ProfileEvent_ExecutableUserDefinedFunctionInputBytes` UInt64 COMMENT 'Total bytes written to the stdin of executable and executable pool user-defined function child processes.',
    `ProfileEvent_ExecutableUserDefinedFunctionOutputBytes` UInt64 COMMENT 'Total bytes read from the stdout of executable and executable pool user-defined function child processes.',
    ...
    `ProfileEvent_JoinBuildPostProcessingMicroseconds` UInt64 COMMENT 'Elapsed time of post-processing steps after building the right JOIN side.',
    `ProfileEvent_AIInputTokens` UInt64 COMMENT 'Total prompt tokens consumed across all AI function calls in the query.',
    `ProfileEvent_AIOutputTokens` UInt64 COMMENT 'Total completion tokens consumed across all AI function calls in the query.',
    `ProfileEvent_AIAPICalls` UInt64 COMMENT 'Number of HTTP requests dispatched to AI providers.',
    `ProfileEvent_AIRowsProcessed` UInt64 COMMENT 'Number of rows that received an AI result.',
    `ProfileEvent_AIRowsSkipped` UInt64 COMMENT 'Number of rows that received a default value due to quota or error.',
    ...
    `CurrentMetric_DistributedIndexAnalysisThreadsScheduled` Int64 COMMENT 'Number of queued or active jobs in the distributed index analysis thread pool.',
    ...
    `CurrentMetric_IcebergCompactionThreads` Int64 COMMENT 'Number of threads in the IcebergCompaction thread pool.',
    `CurrentMetric_IcebergCompactionThreadsActive` Int64 COMMENT 'Number of threads in the IcebergCompaction thread pool running a task.',
    `CurrentMetric_IcebergCompactionThreadsScheduled` Int64 COMMENT 'Number of queued or active jobs in the IcebergCompaction thread pool.',
    `CurrentMetric_IcebergCompactionSnapshots` Int64 COMMENT 'Number of iceberg compactions.',
    ...
    `CurrentMetric_KeeperReadThreads` Int64 COMMENT 'Number of threads in the threadpool for keeper server reads.',
    `CurrentMetric_KeeperReadThreadsActive` Int64 COMMENT 'Number of active threads in the threadpool for keeper server reads.',
    `CurrentMetric_KeeperReadThreadsScheduled` Int64 COMMENT 'Number of queued or active jobs in the threadpool for keeper server reads. Meaningless metric, the actual read tasks on this thread pool are scheduled through a different mechanism.',
    ...
    `CurrentMetric_FilesystemCacheOvercommitUsers` Int64 COMMENT 'Number of users tracked by the overcommit filesystem cache eviction policy',
    ...
    `CurrentMetric_AvroSchemaCacheCells` Int64 COMMENT 'Number of cached Avro schemas, including both registered and fetched schemas.',
    ...
    `CurrentMetric_SerializationCacheBytesInMemoryAllocated` Int64 COMMENT 'Total size of the serialization cache in bytes including keys and overhead from empty slots',
    `CurrentMetric_SerializationCacheBytesInMemory` Int64 COMMENT 'Total size of the serialization cache in bytes including only the values',
    `CurrentMetric_SerializationCacheCount` Int64 COMMENT 'Total number of entries in the serialization cache',
    ...
    `histograms.metric` Array(LowCardinality(String)) COMMENT 'Names of histogram families snapshotted in this row.',
    `histograms.labels` Array(Map(LowCardinality(String), LowCardinality(String))) COMMENT 'Per-entry label maps.',
    `histograms.histogram` Array(Map(Float64, UInt64)) COMMENT 'Per-entry cumulative bucket counts keyed by upper bound; +Inf is the final entry and equals count.',
    `histograms.count` Array(UInt64) COMMENT 'Per-entry total observation counts.',
    `histograms.sum` Array(Float64) COMMENT 'Per-entry sums of observed values.',
